...
const Home = ({ user, setUser, error }) => {
  ...
  const updateInventory = async (id, card, amount) => {
    try {
      await axios.put(`${API_BASE}/api/users/admin/inventory/${id}`, {
        card,
        amount,
      });
      fetchUsers();
  ...
  const updateResources = async (id, resource, amount) => {
    try {
      await axios.put(`${API_BASE}/api/users/admin/resources/${id}`, {
        resource,
        amount,
      });
      fetchUsers();
  ...
  const updateBuildings = async (id, type, amount) => {
    try {
      await axios.put(`${API_BASE}/api/users/admin/buildings/${id}`, {
        type,
        amount,
      });
      fetchUsers();
  ...
  const handleMysteryCard = async () => {
    try {
      if (user.Token <= 0) return alert("Not enough tokens!");

      const cards = Object.keys(cardImages);
      const randomCard = cards[Math.floor(Math.random() * cards.length)];

      await axios.put(`${API_BASE}/api/users/inventory/${user._id}`, {
        card: randomCard,
        amount: (inventory[randomCard] || 0) + 1,
      });

      await axios.put(`${API_BASE}/api/users/tokens/${user._id}`, {
        amount: -1,
      });
                  ...
                  <>
                    ...
                    <div className="text-left bg-white/80 p-4 rounded-lg mb-6 shadow-md">
                      ...
                      {Object.keys(cardImages).map((card) => (
                        <div
                          key={card}
                          className="flex items-center gap-3 bg-white rounded-md p-2 mb-2 shadow-sm border"
                        >
                          <img src={cardImages[card]} className="w-8 h-8" />
                          <span className="capitalize flex-1">
                            {card.replace(/([A-Z])/g, " $1")}:
                          </span>
                          <span>{inventory?.[card] ?? 0}</span>
                        </div>
                      ))}
                    ...
                    <div className="text-left bg-white/80 p-4 rounded-lg mb-6 shadow-md">
                      ...
                      {Object.keys(resourceImages).map((res) => (
                        <div
                          ...
                          className="flex items-center gap-3 bg-white rounded-md p-2 mb-2 shadow-sm border"
                        >
                          <img src={resourceImages[res]} className="w-8 h-8" />
                          <span className="capitalize flex-1">{res}</span>
                          <span>{user?.resources?.[res] ?? 0}</span>
                        </div>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <div>
                      ...
                      {users
                        ...
                        .map((u) => {
                          const tCards = sumCards(u.inventory || {});
                          const tRes = sumResources(u.resources || {});
                          const tTotal = tCards + tRes;
                          ...
                          return (
                            ...
                            >
                              ...
                              <span>
                                🏠 {u.house ?? 0} | 🏡 {u.village ?? 0} | 🛣️{" "}
                                {u.roads ?? 0} | 🏆 {u.score ?? 0} | 🌾 {tRes} |
                                🃏 {tCards} | 📊 {tTotal}
                              </span>
                            </div>
                          );
              ...
              <div>
                ...
                {users
                  ...
                  .map((u) => (
                    <div
                      key={u._id}
                      className="border p-4 rounded mb-4 bg-white/90"
                    >
                      {/* Edit user */}
                      {editingUser === u._id ? (
                        <div>
                      ...
                      <div className="mt-4">
                        ...
                        {Object.entries(u.inventory || {}).map(
                          ([card, count]) => (
                            <div
                              key={card}
                            ...
                            >
                              ...
                              <div className="flex gap-2">
                                <button
                                  onClick={() =>
                                    updateInventory(u._id, card, count - 1)
                                  }
                                  className="px-2 bg-red-500 text-white rounded"
                                >
                                  -
                      ...
                      <div className="mt-4">
                        ...
                        {Object.entries(u.resources || {}).map(
                          ([res, amount]) => (
                            <div
                              key={res}
                              className="flex justify-between p-2 border rounded bg-white mb-1"
                            >
                              ...
                              <div className="flex gap-2">
                                <button
                                  onClick={() =>
                                    updateResources(u._id, res, amount - 1)
                                  }
                                  className="px-2 bg-red-500 text-white rounded"
                                >
                                  -
